...
class TwentynewsProcessor(DataProcessor):
    """Processor for the 20news data set."""

    def __init__(self):
        self.labels = ['alt.atheism',
                       'comp.graphics',
    ...
    def get_examples(self, data_dir, data_type):
        """See base class."""
        with open(F"{data_dir}/hedwig_20news_{data_type}.json") as fo:
            lines = json.load(fo)
        examples = []
        for (i, line) in enumerate(lines):
            guid = [self.data_types.index(data_type), i]
...
def create_interaction_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    with tf.variable_scope("loss"):
        label_middle = tf.layers.dense(output_label_logits, units=hidden_size, activation=tf.tanh)
        label_final = tf.layers.dense(label_middle, units=1, activation=tf.tanh)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate, num_train_steps, num_warmup_steps,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            logging_hook = tf.train.LoggingTensorHook({"loss": total_loss, "label_ids": label_ids}, every_n_iter=40)
